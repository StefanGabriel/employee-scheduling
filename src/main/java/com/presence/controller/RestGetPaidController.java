...
@Controller
public class RestGetPaidController {
    @Autowired
    UsersInfoService userInfo;

    //pagina returnata este valabila doar pentru angajatii
    //pentru care camul "is_manager" este setat true
    @GetMapping("/rest_get_paid")
    public String register(Model model) {
        model.addAttribute("isman", userInfo.isManager());
        return "rest_get_paid";
    }

    //preluam datele introduse de utilizator si le ducem in baza de date
    //in tabela corespunzatoare
    //in aceasta pagina utiliatorul isi poate crea cereri de concediu de odihna
    @RequestMapping(value = "/rest_get_paid/send", method = RequestMethod.POST)
    public RedirectView addRestGetPaid(@RequestParam(value = "start_date") String start_date,
        ...
        if (StringUtils.isBlank(start_date)) {
            ...
            return new RedirectView("/rest_get_paid");
        }
        ...
        userInfo.addRestGetPaid(start_date, finish_date, days_number, manager_name);
