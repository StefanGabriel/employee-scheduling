...
public class StoreRestController {
    ...
    //"cu plata" asociat acestuia, si poate in acelasi timp si incarca fisiere in acel folder
    @GetMapping(value = "/storerest")
    public String storerest(Model model) {
        List<Path> lodf = new ArrayList<>();
        List<HRefModel> uris = new ArrayList<>();
        storageService.setFolder("cu_plata");
        ...
        try {
            ...
            for(Path pt : lodf) {
                ...
                href.setHref(MvcUriComponentsBuilder
                        .fromMethodName(StoreRestController.class, "serveFile", pt.getFileName().toString())
                        .build()
                        .toString());
    ...
    @RequestMapping(value = "/more/storerest", method = RequestMethod.POST)
    public String handleFileUpload(@RequestParam("file") MultipartFile file, RedirectAttributes redirectAttributes) {
        storageService.store(file);
        redirectAttributes.addFlashAttribute("message", "You successfully uploaded " + file.getOriginalFilename() + "!");
        return "redirect:/storerest";
    }
}
